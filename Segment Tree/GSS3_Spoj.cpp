...
struct SegTree{ int pref, suff, maxsum, sum;};
SegTree tree[N<<2];
...
void update(int node, int ind, int val, int low, int high) {
	if (low == high && low == ind) {
		tree[node].pref = tree[node].suff = 
		tree[node].sum = tree[node].maxsum = val;
		arr[ind] = val;
		return;
	}
	...
	int mid = low + ((high - low) >> 1);
	if (ind >= low && ind <= mid) {
		update(l, ind, val, low, mid);
	}
	else update(r, ind, val, mid + 1, high);
...
int main () {
	...
	for (int i = 0; i < m; i++) {
		...
		if (!code) {
			update(1, x, y, 0, n - 1);
			/*for (int i = 0; i < n; i++) {
				printf("%d, ", query(1, 0, n - 1, i, i).maxsum);
			}
			puts("");*/
		}
		else {
